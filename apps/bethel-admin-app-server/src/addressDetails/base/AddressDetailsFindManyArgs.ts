/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { AddressDetailsWhereInput } from "./AddressDetailsWhereInput";
import { IsOptional, ValidateNested, IsInt } from "class-validator";
import { Type } from "class-transformer";
import { AddressDetailsOrderByInput } from "./AddressDetailsOrderByInput";

@ArgsType()
class AddressDetailsFindManyArgs {
  @ApiProperty({
    required: false,
    type: () => AddressDetailsWhereInput,
  })
  @IsOptional()
  @ValidateNested()
  @Field(() => AddressDetailsWhereInput, { nullable: true })
  @Type(() => AddressDetailsWhereInput)
  where?: AddressDetailsWhereInput;

  @ApiProperty({
    required: false,
    type: [AddressDetailsOrderByInput],
  })
  @IsOptional()
  @ValidateNested({ each: true })
  @Field(() => [AddressDetailsOrderByInput], { nullable: true })
  @Type(() => AddressDetailsOrderByInput)
  orderBy?: Array<AddressDetailsOrderByInput>;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  skip?: number;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  take?: number;
}

export { AddressDetailsFindManyArgs as AddressDetailsFindManyArgs };
